...
ChkSlider.prototype = {
    init: function({divId, speed, duration, animation, direction, isNavigation, isArrowButton, isPauseMouseOver}) {
        ...
        //슬라이드가 2개보다 작을 때 return false
        if( _this.length < 2 ) return false;
    ...
    slideItem: function(currentIndex, direction) {
        ...
        if (moveDirection == "left") {
            _this.$visual.eq(_this.current).css({
                left: '0'
            }).stop().animate({
                left: '-100%'
            });
            _this.$visual.eq(currentIndex).css({
                left: '100%'
            }).stop().animate({
                left: '0'
            });
    ...
    createNavigation: function() {
        ...
        for (i = 0; i < _this.length; i++) {
            ...
            _this.$div.find("nav").find("button").eq(i).attr({
                "data-idx": (i + 1)
            });
        }
